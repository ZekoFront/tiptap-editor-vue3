<template>
    <div :class="['editor-image-bubble--menu']" v-if="editor">
        <NIcon
            class="bubble-icon-item"
            size="21"
            :title="item.title"
            :color="displayState === item.state ? ThemeColor : ''"
    ...
    <n-modal v-model:show="showCropModal" class="crop-image-modal">
        <n-card style="width: 700px" :bordered="false" size="huge" role="dialog" aria-modal="true">
            <div class="crop-preview-box">
                ...
                <div class="preview-image" v-if="previewImage" v-html="previewImage?.html"></div>
            </div>
...
const displayList = shallowRef([
    { title: "裁剪", state: "crop", icon: Icons.CuttingIcon },
    { title: "删除", state: "del", icon: Icons.TrashCanIcon },
    { title: "内联元素", state: ImageDisplay.INLINE, icon: Icons.InlineIcon },
    { title: "块元素", state: ImageDisplay.BREAK_TEXT, icon: Icons.BlockIcon },
    { title: "左对齐", state: ImageDisplay.FLOAT_LEFT, icon: Icons.LeftAlignmentIcon },
    { title: "右对齐", state: ImageDisplay.FLOAT_RIGHT, icon: Icons.RightAlignmentIcon }
]);
