<template>
    <n-popover
        trigger="click"
        content-class="toolbar-table-picker__popover"
        style="max-height: 340px; max-width: 335px"
    >
        <template #trigger>
        ...
        <div class="table-drawing">
            <p style="margin: 5px 0; display: flex; align-items: center">
                <strong>{{ tbOptions.row }}</strong
                >行x<strong>{{ tbOptions.column }}</strong
                >列, 表头: <input type="checkbox" v-model="tbOptions.isWithHeaderRow" />
                <a href="#" class="confirm__btn" @click.stop="handleTable">确定</a>
            </p>
            <div
                ref="tableFilterWrapRef"
                class="table-drawing__area"
